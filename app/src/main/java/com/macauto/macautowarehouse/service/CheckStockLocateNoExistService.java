...
public class CheckStockLocateNoExistService extends IntentService {
    public static final String TAG = "CheckStockLocateNo";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            ...
            if (envelope.bodyIn instanceof SoapFault) {
                ...
                if (!is_exist) {
                    checkResultIntent = new Intent(Constants.ACTION.ACTION_PRODUCT_CHECK_STOCK_LOCATE_NO_EXIST_NO);
                    checkResultIntent.putExtra("CURRENT_INDEX", current_index);
                    sendBroadcast(checkResultIntent);
                } else {
                    checkResultIntent = new Intent(Constants.ACTION.ACTION_PRODUCT_CHECK_STOCK_LOCATE_NO_EXIST_YES);
                    checkResultIntent.putExtra("CURRENT_INDEX", current_index);
                    sendBroadcast(checkResultIntent);
                }
            ...
            e.printStackTrace();
            Intent getFailedIntent = new Intent(Constants.ACTION.ACTION_PRODUCT_CHECK_STOCK_LOCATE_NO_EXIST_FAILED);
            getFailedIntent.putExtra("CURRENT_INDEX", current_index);
