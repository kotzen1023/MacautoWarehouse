...
import static com.macauto.macautowarehouse.data.WebServiceParse.parseToArrayString;
...
public class GetLotCodeService extends IntentService {
    public static final String TAG = "GetLotCodeService";
    ...
    private static final String METHOD_NAME = "get_lot_code_ver_2018"; // 方法名稱

    private static final String SOAP_ACTION1 = "http://tempuri.org/get_lot_code_ver_2018"; // SOAP_ACTION
    //normal port 8000, test port 8484
    //private static final String URL = "http://172.17.17.244:"+web_soap_port+"/service.asmx"; // 網址

    public GetLotCodeService() {
        super("GetLotCodeService");
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        String bar_code = intent.getStringExtra("BAR_CODE");
        ...
        if (intent.getAction() != null) {
            if (intent.getAction().equals(Constants.ACTION.ACTION_GET_LOT_CODE_ACTION)) {
                Log.i(TAG, "ACTION_GET_LOT_CODE_ACTION");
            }
        }
        ...
        try {
            ...
            if (envelope.bodyIn instanceof SoapFault) {
                ...
                Log.e(TAG, str);
                Intent getFailedIntent = new Intent(Constants.ACTION.ACTION_GET_LOT_CODE_FAILED);
                ...
                printArray = parseToArrayString(resultsRequestSOAP);

                //append_record(String.valueOf(resultsRequestSOAP)+"\n\n\n\n", "test");

                if (printArray.size() > 0) {
                    Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_GET_LOT_CODE_SUCCESS);
                    sendBroadcast(getSuccessIntent);
                } else {
                    Intent getEmptyIntent = new Intent(Constants.ACTION.ACTION_GET_LOT_CODE_EMPTY);
