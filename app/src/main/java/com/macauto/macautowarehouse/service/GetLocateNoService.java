...
public class GetLocateNoService extends IntentService {
    public static final String TAG = "GetLocateNoService";
    ...
    private static final String METHOD_NAME = "get_locate_no"; // 方法名稱
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent.getAction() != null) {
            if (intent.getAction().equals(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_LOCATE_NO_ACTION)) {
                Log.i(TAG, "ACTION_ALLOCATION_SEND_MSG_GET_LOCATE_NO_ACTION");
            }
        }
        ...
        try {
            ...
            if (envelope.bodyIn instanceof SoapFault) {
                ...
                Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_LOCATE_NO_EMPTY);
                sendBroadcast(getSuccessIntent);
            } else {
                SoapObject resultsRequestSOAP = (SoapObject) envelope.bodyIn;
                Log.e(TAG, String.valueOf(resultsRequestSOAP));

                SoapObject s_deals = (SoapObject) resultsRequestSOAP.getProperty("get_locate_noResult");


                locateNoTable = parseXmlToDataTable(s_deals);

                if (locateNoTable != null) {
                    if (locateNoTable.Rows.size() == 0) {
                        Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_LOCATE_NO_EMPTY);
                        sendBroadcast(getSuccessIntent);
                    } else {
                        Log.e(TAG, "locateNoTable.Rows = "+locateNoTable.Rows.size());
                        for (int i=0; i < locateNoTable.Rows.size(); i++) {
                            Log.e(TAG, "value["+i+"] = "+locateNoTable.getValue(i, 0).toString() );
                            locateList.add(locateNoTable.getValue(i, 0).toString());

                        }

                        Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_LOCATE_NO_SUCCESS);
            ...
            e.printStackTrace();
            Intent getFailedIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_LOCATE_NO_FAILED);
