...
public class GetMadeInfoService extends IntentService {
    public static final String TAG = "GetMadeInfoService";
    ...
    private static final String METHOD_NAME = "get_made_info"; // 方法名稱

    private static final String SOAP_ACTION1 = "http://tempuri.org/get_made_info"; // SOAP_ACTION

    //private static final String URL = "http://172.17.17.244:8484/service.asmx"; // 網址

    public GetMadeInfoService() {
        super("GetMadeInfoService");
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent.getAction() != null) {
            if (intent.getAction().equals(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_MADE_INFO_ACTION)) {
                Log.i(TAG, "ACTION_ALLOCATION_SEND_MSG_GET_MADE_INFO_ACTION");
            }
        }
        ...
        try {
            ...
            if (envelope.bodyIn instanceof SoapFault) {
                ...
                Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_MADE_INFO_EMPTY);
                sendBroadcast(getSuccessIntent);
            } else {
                SoapObject resultsRequestSOAP = (SoapObject) envelope.bodyIn;
                Log.e(TAG, String.valueOf(resultsRequestSOAP));

                SoapObject s_deals = (SoapObject) resultsRequestSOAP.getProperty("get_made_infoResult");


                madeInfoTable = parseXmlToDataTable(s_deals);

                if (madeInfoTable != null) {
                    if (madeInfoTable.Rows.size() == 0) {
                        Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_MADE_INFO_EMPTY);
                        sendBroadcast(getSuccessIntent);
                    } else {
                        Log.e(TAG, "madeInfoTable.Rows = "+madeInfoTable.Rows.size());
                        /*Log.e(TAG, "madeInfoTable.Rows = "+madeInfoTable.Rows.size());
                        for (int i=0; i < madeInfoTable.Rows.size(); i++) {
                            Log.e(TAG, "value["+i+"] = "+madeInfoTable.getValue(i, 0).toString() );


                        }*/
                        String s_part_no = madeInfoTable.getValue(0, 0).toString();
                        String s_part_desc = madeInfoTable.getValue(0, 1).toString();
                        String s_pdt_qty = madeInfoTable.getValue(0, 2).toString();
                        String s_pdted_qty = madeInfoTable.getValue(0, 3).toString();

                        Intent getSuccessIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_MADE_INFO_SUCCESS);
            ...
            e.printStackTrace();
            Intent getFailedIntent = new Intent(Constants.ACTION.ACTION_ALLOCATION_SEND_MSG_GET_MADE_INFO_FAILED);
